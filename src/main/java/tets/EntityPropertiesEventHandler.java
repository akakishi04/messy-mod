...
public class EntityPropertiesEventHandler {
    ...
    public void onEntityConstructing(EntityEvent.EntityConstructing event) {
        if (event.entity instanceof EntityPlayer) {
            EnPro.register((EntityPlayer)event.entity);
        }
    }
   ...
   @SubscribeEvent
    ...
    public void onClonePlayer(net.minecraftforge.event.entity.player.PlayerEvent.Clone event) {
        ...
        if (event.wasDeath) {
            //古いカスタムデータ
            IExtendedEntityProperties oldEntityProperties = event.original.getExtendedProperties(EnPro.EXT_PROP_NAME);
            //新しいカスタムデータ
            IExtendedEntityProperties newEntityProperties = event.entityPlayer.getExtendedProperties(EnPro.EXT_PROP_NAME);
    ...
    public void respawnEvent(PlayerEvent.PlayerRespawnEvent event) {
        if (!event.player.worldObj.isRemote) {
            PacketHandler.INSTANCE.sendTo(new MPP(event.player), (EntityPlayerMP)event.player);
        }
    }
}
